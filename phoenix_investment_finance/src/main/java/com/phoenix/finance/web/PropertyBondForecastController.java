...
public class PropertyBondForecastController implements com.phoenix.finance.web.Controller {
	...
	public PropertyBondForecast getModel(HttpServletRequest req) {
		if (isFirstTime(req)) {
			...
			String termParam = req.getParameter("term");
			String bondFundParam = req.getParameter("bondFund");
			String bondType = req.getParameter("bondType");
			PropertyBondForecast propertyBondForecast = createModel(principalParam, interestRateParam, termParam,
					bondFundParam, bondType);
			return propertyBondForecast;
		} else {
			String principalParam = req.getParameter("basePrincipal");
			String interestRateParam = req.getParameter("baseInterestRate");
			String termParam = req.getParameter("baseTerm");
			String bondFundParam = req.getParameter("baseBondFund");
			String bondType = req.getParameter("baseBondType");
			PropertyBondForecast propertyBondForecast = createModel(principalParam, interestRateParam, termParam,
					bondFundParam, bondType);
			return propertyBondForecast;
		}
		// Money principal = new Money(new BigDecimal(principalParam));
		// BigDecimal interestRate = new BigDecimal(interestRateParam);
		// int term = Integer.parseInt(termParam);
		// BondFund bondFund = getBondFund(bondFundParam);
		// PropertyBond bond = new PropertyBond(principal, interestRate, term,
		// bondFund, bondType);
		// PropertyBondForecast propertyBondForecast = new
		// PropertyBondForecast(bond);
		//
	...
	private PropertyBondForecast createModel(String principalParam, String interestRateParam, String termParam,
			String bondFundParam, String bondType) {
		Money principal = new Money(new BigDecimal(principalParam));
		BigDecimal interestRate = new BigDecimal(interestRateParam);
		int term = Integer.parseInt(termParam);
		BondFund bondFund = getBondFund(bondFundParam);
